    ...
    prisma.user
      ...
      .then((v) => {
        if ((v?.permission as string[]).includes("admin")) {
          ...
          let original = req.query.original as string;
          ...
          prisma.problem
            ...
            .then((v) => {
              if (v == null) {
                res.send(
                  JSON.stringify({
                    err: `Problem with id ${original} doesn't exists`,
                  })
                );
                resolve();
                return;
          ...
          res.end(
            JSON.stringify({
              err: "You do not have permission to update a problem",
            })
          );
        }
      });
